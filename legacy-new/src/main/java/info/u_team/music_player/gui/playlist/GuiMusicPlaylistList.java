...
public class GuiMusicPlaylistList extends BetterScrollableList<GuiMusicPlaylistListEntry> {
	...
	public void addAllEntries() {
		...
		if (!tracksLoaded) {
			...
			tracksLoaded = true;
			if (selectIndex >= 0 && selectIndex < children().size()) {
				super.setSelected(children().get(selectIndex));
			}
		}
	}
	...
	@Override public boolean mouseClicked(MouseButtonEvent event,boolean doubleClick){if(event.button()==0&&event.x()<=getX()+18){final WrappedObject<String> uri=sourceAt(event.x(),event.y());if(uri!=null){dragSource=uri;dragTarget=uri;return true;}}return super.mouseClicked(event,doubleClick);}
	@Override public boolean mouseDragged(MouseButtonEvent event,double dragX,double dragY){if(dragSource!=null&&event.button()==0){final WrappedObject<String> target=sourceAt(event.x(),event.y());if(target!=null)dragTarget=target;return true;}return super.mouseDragged(event,dragX,dragY);}
	@Override public boolean mouseReleased(MouseButtonEvent event){if(dragSource!=null&&event.button()==0){final WrappedObject<String> source=dragSource,target=dragTarget;dragSource=null;dragTarget=null;if(source!=target&&playlist.moveTo(source,target))updateAllEntries();return true;}return super.mouseReleased(event);}
