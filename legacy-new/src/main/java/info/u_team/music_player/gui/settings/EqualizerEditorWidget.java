...
final class EqualizerEditorWidget implements Renderable, GuiEventListener, NarratableEntry {
	...
	@Override public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
		...
		final float[] gains = settings.getEqualizerGains(), positions = settings.getEqualizerPositions();
		...
		for (int band = 0; band < gains.length; band++) {
			...
			GuiTextCompat.draw(graphics, Minecraft.getInstance().font, label, labelX, y + plotHeight + 3, 0xFFCBD5DF, false);
		}
	}

	private void renderBars(GuiGraphics graphics, float[] gains, int plotHeight) {
		final int zeroY = gainY(0F, plotHeight);
		for (int band = 0; band < gains.length; band++) {
			final int center = bandX(band), knobY = gainY(gains[band], plotHeight);
	...
	private void renderCurve(GuiGraphics graphics, float[] gains, float[] positions, int plotHeight) {
		int previousY = gainY(interpolate(gains, positions, 0F), plotHeight);
		for (int pixel = 0; pixel < width; pixel++) {
			final int currentY = gainY(interpolate(gains, positions, pixel / Math.max(1F, width - 1F)), plotHeight);
			graphics.fill(x + pixel, Math.min(previousY, currentY), x + pixel + 1, Math.max(previousY, currentY) + 2, 0xFF75E0B5);
			previousY = currentY;
		}
		for (int band = 0; band < gains.length; band++) {
			final int center = pointX(positions[band]), pointY = gainY(gains[band], plotHeight);
			graphics.fill(center - 4, pointY - 4, center + 5, pointY + 5, band == draggedBand ? 0xFFFFCC66 : 0xFFB8F3DB);
		}
	}
	...
	@Override public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
		...
		if (doubleClick) settings.setEqualizerPoint(draggedBand, settings.getEqualizerPositions()[draggedBand], 0F); else update(event.x(), event.y());
		return true;
	}
	@Override public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) { if (draggedBand < 0 || event.button() != 0) return false; update(event.x(), event.y()); return true; }
	@Override public boolean mouseReleased(MouseButtonEvent event) { if (event.button() != 0 || draggedBand < 0) return false; MusicPlayerManager.getSettingsManager().getSettings().commitEqualizerEdit(); draggedBand = -1; return true; }
	...
	private void update(double mouseX, double mouseY) {
		...
		final Settings settings = MusicPlayerManager.getSettingsManager().getSettings();
		if (settings.getEqualizerMode() == EqualizerMode.PARAMETRIC) settings.previewEqualizerPoint(draggedBand, (float) Math.clamp((mouseX - x) / Math.max(1D, width - 1D), 0D, 1D), gain); else settings.previewEqualizerGain(draggedBand, gain);
	}
