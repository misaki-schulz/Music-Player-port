// Legacy Fabric event bridge for Minecraft 1.21.9-1.21.11; see NOTICE.
...
public class MusicPlayerEventHandler {
	...
	private static void onScreenKeyboard(Screen screen, KeyEvent event) {
		final boolean open = MusicPlayerKeys.OPEN.matches(event);
		if (!open && !settingsManager.getSettings().isKeyWorkInGui()) {
			return;
		}
		handleKeyboard(Minecraft.getInstance(), open, MusicPlayerKeys.HUD_CONTROLS.matches(event), MusicPlayerKeys.PAUSE.matches(event),
				MusicPlayerKeys.SKIP_FORWARD.matches(event), MusicPlayerKeys.SKIP_BACK.matches(event));
	}
