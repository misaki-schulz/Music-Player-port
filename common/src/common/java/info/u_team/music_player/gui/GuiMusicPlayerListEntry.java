...
class GuiMusicPlayerListEntry extends BetterScrollableListEntry<GuiMusicPlayerListEntry> {
	...
	public void render(GuiGraphicsExtractor guiGraphics, int slotIndex, int entryY, int entryX, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float partialTicks) {
		...
		guiGraphics.text(minecraft.font, playlist.getEntrySize() + " " + getTranslation(playlist.getEntrySize() > 1 ? GUI_PLAYLISTS_ENTRIES : GUI_PLAYLISTS_ENTRY), entryX + 5, entryY + 30, 0xFFFFFFFF, false);
		
		playPlaylistButton.setX(entryWidth - 65);
		playPlaylistButton.setY(entryY + 12);
		playPlaylistButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTicks);
		
		openPlaylistButton.setX(entryWidth - 40);
		openPlaylistButton.setY(entryY + 12);
		openPlaylistButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTicks);
		
		deletePlaylistButton.setX(entryWidth - 15);
		deletePlaylistButton.setY(entryY + 12);
		deletePlaylistButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTicks);
	}
