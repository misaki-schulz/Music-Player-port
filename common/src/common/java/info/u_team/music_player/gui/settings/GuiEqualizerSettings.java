...
public final class GuiEqualizerSettings extends BetterScreen {
	...
	private EqualizerEditorWidget editor;
	...
	@Override protected void init() {
		...
		final EqualizerPresetManager presets = MusicPlayerManager.getEqualizerPresetManager();
		final int actionWidth = Math.max(45, (width - 16) / 5);
		final UButton preset = addRenderableWidget(new UButton(8, 36, actionWidth - 3, 20, Component.literal(presets.getSelected())));
		preset.setPressable(() -> preset.setMessage(Component.literal(presets.selectNext(settings))));
		addRenderableWidget(new UButton(8 + actionWidth, 36, actionWidth - 3, 20, text("gui.equalizer.save"), button -> preset.setMessage(Component.literal(presets.saveCurrent(settings)))));
		addRenderableWidget(new UButton(8 + actionWidth * 2, 36, actionWidth - 3, 20, text("gui.equalizer.delete"), button -> { if (presets.deleteSelected()) { settings.resetEqualizer(); preset.setMessage(Component.literal(presets.getSelected())); } }));
		addRenderableWidget(new UButton(8 + actionWidth * 3, 36, actionWidth - 3, 20, text("gui.equalizer.import"), button -> PlaylistFileDialogs.chooseImport(path -> { try { status = getTranslation("gui.equalizer.imported", presets.importFile(path)); } catch (java.io.IOException exception) { status = getTranslation("gui.equalizer.import_failed"); } })));
		addRenderableWidget(new UButton(8 + actionWidth * 4, 36, actionWidth - 3, 20, text("gui.equalizer.export"), button -> PlaylistFileDialogs.chooseExport("music-player-eq-presets.json", path -> { try { presets.exportFile(path); status = getTranslation("gui.equalizer.exported"); } catch (java.io.IOException exception) { status = getTranslation("gui.equalizer.export_failed"); } })));
		editor = new EqualizerEditorWidget(12, 66, width - 24, Math.max(60, height - 78));
		addWidget(editor);
	}
	@Override public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) { super.extractRenderState(graphics, mouseX, mouseY, partialTick); editor.extractRenderState(graphics, mouseX, mouseY, partialTick); graphics.text(font, status, 12, 58, 0xFFB8E986, false); }
