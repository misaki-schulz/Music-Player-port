...
public class GuiControls extends AbstractContainerEventHandler implements BetterNestedGui, Renderable, NarratableEntry {
	...
	public GuiControls(Screen gui, int y, int width) {
		...
		if (!isSettings) {
			...
			nowPlayingButton.setPressable(() -> MinecraftGuiCompat.setScreen(mc, new GuiNowPlaying(gui)));
		}
	...
	public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		poseStack.identity();
		buttons.forEach(button -> button.extractRenderState(guiGraphics, mouseX, mouseY, partialTicks));
		songProgress.extractRenderState(guiGraphics, mouseX, mouseY, partialTicks);
	...
	public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent event, boolean doubleClick) {
		if (manager.getCurrentTrack() != null && event.button() == 0 && (checkClick(titleRender, event.x(), event.y()) || checkClick(authorRender, event.x(), event.y()))) {
