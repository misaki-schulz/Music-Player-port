...
public class DependencyManager {
	...
	public static void load() {
		...
		if (devPath != null) {
			paths = Stream.of(devPath.split(Pattern.quote(File.pathSeparator))) //
	...
	private static Set<Path> findJarFilesInDev(Path path) {
		try (final Stream<Path> stream = Files.walk(path)) {
			...
			LOGGER.error(MARKER_LOAD, "When searching for jar files in dev an exception occurred.", ex);
		}
	...
	private static Set<Path> findJarFilesInJar(String folder) {
		try {
			final ModContainer container = FabricLoader.getInstance().getModContainer(MusicPlayerReference.MODID).orElseThrow(IllegalStateException::new);
			final Path dependencyFolder = container.findPath(folder).orElseThrow(IllegalStateException::new);
			try (final Stream<Path> stream = Files.walk(dependencyFolder)) {
				return filterPackedFiles(stream);
			}
		} catch (final IOException | IllegalStateException ex) {
			LOGGER.error(MARKER_LOAD, "When searching for jar files in jar an exception occurred.", ex);
		}
		return Collections.emptySet();
	}
