...
public class LoadedTracks {
	...
	public LoadedTracks(WrappedObject<String> uri, String errorMessage) {
		this.uri = uri;
		this.errorMessage = errorMessage == null || errorMessage.isBlank() ? "Unable to load track" : errorMessage;
	}
	...
	public IAudioTrack getFirstTrack() {
		if (isTrack()) {
			return track;
		} else if (isTrackList() && !trackList.getTracks().isEmpty()) {
